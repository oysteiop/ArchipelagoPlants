##################################################################################################
# ARCHIPELAGO PLANTS: OUTPUT OF EXTINCTION MODEL
##################################################################################################
...
setwd("F:/HelsinkiData23102019/archipelago/hmsc/Rcode/extinction")
load("modelNS.RData")
...
str(post$Omega[[1]])
set.seed(1)
ss=sample(1:215296, 1000, replace=F)
...
mean(R2,na.rm=T) #0.16
range(R2,na.rm=T) #0 - 0.57
mean(AUC,na.rm=T) #0.85
range(AUC,na.rm=T) #.43 - 1.00
...
pdf("plots/varpartExt.pdf",height=5,width=60)
plotVariancePartitioning(m, VP = VP)
dev.off()

### - Association networks ordered by taxonomy - ####
OmegaCorExt = computeAssociations(m)
#save(OmegaCorExt, file="OmegaCorExt.RData")

load(file="OmegaCorExt.RData")
...
orderC[1:5,1:5]
orderOmega=OmegaCorExt[[1]]$mean[tree$tip.label,tree$tip.label]
orderOmega[1:5,1:5]

colnames(orderOmega)==colnames(orderC)

cor(c(orderC),c(orderOmega)) #0.016

plotOrderExt=match(tree$tip.label,colnames(OmegaCorExt[[1]]$mean))
