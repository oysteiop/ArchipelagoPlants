...
plot(xx,R2[seq(1,1753,3)],las=1, main=paste("Historical occurrence: Mean = ", round(mean(R2[seq(1,1753,3)],na.rm=T),2), sep=""),ylim = c(0,1),xlab = "Historical prevalence", ylab=expression(paste("Tjur's ", r^2)))
plot(xx,R2[seq(2,1754,3)],las=1, main=paste("Colonisation probability: Mean = ", round(mean(R2[seq(2,1754,3)],na.rm=T),2), sep=""),ylim = c(0,1),xlab = "Historical prevalence", ylab=expression(paste("Tjur's ", r^2)))
plot(xx,R2[seq(3,1755,3)],las=1, main=paste("Extinction probability: Mean = ", round(mean(R2[seq(3,1755,3)],na.rm=T),2), sep=""),ylim = c(0,1),xlab = "Historical prevalence", ylab=expression(paste("Tjur's ", r^2)))
dev.off()
...
plotGamma(m, post=mgamma,param="Support",supportLevel=.95,covOrder="Vector",covVector=2:4,
          trOrder="Vector",trVector=c(1,4,2,5,3,6),trNamesNumbers=c(T,T),cex=c(.6,.8,.8))
...
# (Set weakly supported coefs to zero)
...
#### -  Boxplot figure - ####
#pdf("col_ext_cors.pdf",height=4,width=4,family="Times")
#par(mfrow=c(1,1))
...
boxplot(dat$corr~dat$group,varwidth=T,col=c("firebrick","navyblue"),at=c(1,2),xlim=c(0,17),xaxt="n",ylab="| r |",las=1,cex=0,main=expression(paste("(a) Residual correlations")))
...
boxplot(dat$corr~dat$group,varwidth=T,col=c("firebrick","navyblue"),at=c(4,5),xlim=c(0,17),xaxt="n",yaxt="n",las=1,cex=0)
...
boxplot(dat$corr~dat$group,varwidth=T,col=c("firebrick","navyblue"),at=c(7,8),xlim=c(0,17),xaxt="n",yaxt="n",cex=0)
...
boxplot(dat$corr~dat$group,varwidth=T,col=c("firebrick","navyblue"),at=c(10,11),xlim=c(0,17),xaxt="n",yaxt="n",cex=0)
...
boxplot(dat$corr~dat$group,varwidth=T,col=c("firebrick","navyblue"),at=c(13,14),xlim=c(0,17),xaxt="n",yaxt="n",cex=0)
...
boxplot(dat$corr~dat$group,varwidth=T,col=c("firebrick","navyblue"),at=c(16,17),xlim=c(0,17),xaxt="n",yaxt="n",cex=0)

axis(1,at=c(1.5,4.5,7.5,10.5,13.5,16.5),labels=rep("",6))
#mtext(c("Hist-Hist","Col-Col","Ext-Ext","Hist-Col","Hist-Ext","Col-Ext"),1,at=c(1.5,4.5,7.5,10.5,13.5,16.5),line=1)
...
for (r in 1:m$nr){
  plotOrder = corrMatOrder(oldCor[[r]]$mean,order="AOE")
  toPlot = ((oldCor[[r]]$support>supportLevel) + (oldCor[[r]]$support<(1-supportLevel))>0)*oldCor[[r]]$mean
  corrplot(toPlot[plotOrder,plotOrder], tl.pos="n",method = "color", type="lower",col=colorRampPalette(c("blue","white","red"))(200),title="Historical occurrence", mar=c(0,0,1,0))
}

for (r in 1:m$nr){
  #plotOrder = corrMatOrder(colCor[[r]]$mean,order="AOE")
  toPlot = ((colCor[[r]]$support>supportLevel) + (colCor[[r]]$support<(1-supportLevel))>0)*colCor[[r]]$mean
  corrplot(toPlot[plotOrder,plotOrder], tl.pos="n",method = "color", type="lower",col=colorRampPalette(c("blue","white","red"))(200),title="Colonisation probability", mar=c(0,0,1,0))
}

for (r in 1:m$nr){
  #plotOrder = corrMatOrder(extCor[[r]]$mean,order="AOE")
  toPlot = ((extCor[[r]]$support>supportLevel) + (extCor[[r]]$support<(1-supportLevel))>0)*extCor[[r]]$mean
  corrplot(toPlot[plotOrder,plotOrder], tl.pos="n",method = "color", type="lower",col=colorRampPalette(c("blue","white","red"))(200),title="Extinction probability", mar=c(0,0,1,0))
}
dev.off()
